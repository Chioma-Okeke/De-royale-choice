...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const tags = Array.from(db.tags.values()).filter((tag) => tag.registrationId === id)
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    // Validate status
    if (status && !["registered", "processing", "completed", "delivered", "cancelled"].includes(status)) {
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (!authResult.success || authResult.user.role !== "admin") {
      return NextResponse.json({ error: "Unauthorized. Only admins can delete registrations" }, { status: 403 })
    }
    ...
    if (registration.status !== "registered" && registration.status !== "cancelled") {
      return NextResponse.json(
    ...
    if (customer) {
      customer.registrations = customer.registrations.filter((regId) => regId !== id)
