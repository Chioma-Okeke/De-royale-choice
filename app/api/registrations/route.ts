...
export async function GET(request: NextRequest) {
    try {
        ...
        let registrations: Registration[] = Array.from(
            db.registrations.values()
        );
        ...
        if (dateFrom) {
            ...
            registrations = registrations.filter(
                (reg) => new Date(reg.dropOffDate) >= fromDate
            );
        }
        ...
        if (dateTo) {
            ...
            registrations = registrations.filter(
                (reg) => new Date(reg.dropOffDate) <= toDate
            );
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        let dailyReport = Array.from(db.dailyReports.values()).find(
